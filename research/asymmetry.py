...
def get_ai_key(rh_key, data):
    return '%s_AI' % data.get_nonhemi_key(rh_key)
...
def get_asymmetry_index(data, key, mask_nan=False):
    ...
    left_key, right_key = data.get_bilateral_hemi_keys(key)

    # Select data within the group
    LH_data = np.asarray(data.data_dict[left_key].tolist())
    RH_data = np.asarray(data.data_dict[right_key].tolist())
